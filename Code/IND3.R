...
# In this case site site a-i are deployment site
departure <- max(tSecond[cL$site==which(letters[]=="l")])

## departure time is "2014-10-15"


######################## Initial path

path <- path$x[twl$Twilight>=(departure-4*24*60*60) & twl$Twilight<=as.POSIXct("2015-02-15"),]
twl  <- subset(twl,Twilight>=(departure-4*24*60*60) & twl$Twilight<=as.POSIXct("2015-02-15"))
...
# this section should not be commented out when the analysis starts
...
fixed[1:4,] <- cbind(TRUE,TRUE) # the points falling into the vicinity of the breeding site
