...
twl <- twilightAdjust(twl,300)
  twl <- twl[twl$Twilight<=as.POSIXct("2015-02-15"),]
...
departure <- max(tSecond[cL$site==which(letters[]=="i")])

## departure time is "2014-10-03"
...
mS <- mergeSites(twl.gl,site=cL$site,distThreshold=300,fixed=fixed,degElevation=90-zenith0)
